...
for i in range(0, num_clusters):
    ...
    for au in df.columns[1:len(df.columns)-1]:
        au_presence = cluster.apply(lambda x: True if x[au] > 0 else False, axis = 1)
        AU_freq[au] = (len(au_presence[au_presence==True].index)/len(cluster.index))*100
...
for au_list in au_clusters:
    ...
    cluster_stds = []
    for au in au_list:
        outliers_up = df_clusters[i][df_clusters[i][au] > df_clusters[i][au].mean() + 2 * df_clusters[i][au].std()]
        outliers_down = df_clusters[i][df_clusters[i][au] < df_clusters[i][au].mean() - 2 * df_clusters[i][au].std()]
        print('----- OUTLIERS ({})-----'.format(au))
        #print(outliers_up)
        #print(outliers_down)
        #print(df_clusters[i][au].mean() + 2 * df_clusters[i][au].std())
        #print(df_clusters[i][au].mean() - 2 * df_clusters[i][au].std())
        if not outliers_up.empty:
            #print(outliers_up)
            df_clusters[i] = df_clusters[i].drop(list(outliers_up.index))
            #print(df_clusters[i])
        if not outliers_down.empty:
            #print(outliers_down)
            df_clusters[i] = df_clusters[i].drop(list(outliers_down.index))
            #print(df_clusters[i])        
