...
exaggerate = float(sys.argv[4])/100
...
for au_list in au_clusters:
    ...
    cluster_stds = []
    for au in au_list:
        exaggerated_au = df_clusters[i].describe(include='all').loc['mean'].loc[au]*(1+exaggerate)
        max_au = df_clusters[i].describe(include='all').loc['max'].loc[au]
        cluster_means.append(exaggerated_au if exaggerated_au <= max_au else max_au)
